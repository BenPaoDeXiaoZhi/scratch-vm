...
class _StackFrame {
   ...
   */
  reuse(warpMode: boolean = this.warpMode): _StackFrame {
    this.reset();
  ...
  /**
   * Put a stack frame object into the recycle bin for empty stackFrame objects.
 ...
 */
class Thread {
  static _StackFrame: typeof _StackFrame;
  target: any;
  stackClick: boolean;
   ...
   */
  pushStack(blockId: any, target?: any) {
    ...
    if (this.stack.length > this.stackFrames.length) {
      ...
      const stackFrame = _StackFrame.create(
   ...
   * @param {string} blockId Block ID to push to stack.
   */
  reuseStackForNextBlock(blockId: any): void {
    ...
    if (globalTarget) {
      this.stackFrames[this.stackFrames.length - 1].executionContext = {
        globalTarget,
      };
    }
  }
  ...
  stopThisScript() {
    ...
    while (blockID !== null) {
      ...
      if (!stackFrame) break;
      const globalTarget =
        stackFrame.executionContext && stackFrame.executionContext.globalTarget;
  ...
  getHatParam(paramName: string): any {
  ...
  getAllparams() {
    const stackFrame = this.peekStackFrame();
    return stackFrame ? stackFrame.params : null;
  }
