...
class Target extends EventEmitter {
   ...
   */
  extensionStorage: Record<string, object> = {};

  /**
   * Currently known values for edge-activated hats.
   * Keys are block ID for the hat; values are the currently known values.
   */
  _edgeActivatedHatValues: Record<string, number> = {};
   ...
   */
  _customState: Record<string, any> = {};

  isStage: boolean = false;
  isOriginal: any;
  // In practice this is a Sprite (or falsey placeholder on the base Target);
  // typed loosely to allow the Sprite-specific members used by callers.
  sprite!: any;
  drawableID!: number|null;
  /**
   * ID of the "original" target for this target (used for clone bookkeeping).
   * The base Target returns null; RenderedTarget overrides this with a getter
   * that resolves to the original clone's id. Implemented as an accessor here
   * (not an initialized field) so that the base constructor never assigns it,
   * which would otherwise throw against RenderedTarget's getter-only property.
   */
  get originalTargetId(): string | null {
    return null;
  }
   ...
   * @constructor
   */
  constructor(runtime: Runtime, blocks?: Blocks, frames?: Frames, id?: string) {
    super();
  ...
  hasEdgeActivatedValue(blockId: string) {
   ...
   */
  findVariableUsage(name: string, type: string): boolean | null {
    if (typeof name !== "string") return null;
    if (typeof type !== "string") type = Variable.SCALAR_TYPE;
    ...
    for (const blockId in this.blocks._blocks) {
      ...
      if (
        ...
        block.fields[fieldKey].value === name
      ) {
        return true;
      }
    }
    ...
    if (!Object.prototype.hasOwnProperty.call(this.variables, id)) {
      ...
      if (!isRemoteOperation) {
        this.runtime.emitTargetVariablesChanged(this.originalTargetId, [
          ...
          type,
          "add",
          { name, value: newVariable.value, isCloud: newVariable.isCloud },
        ]);
      }
    }
  }
  ...
  createComment(
    ...
    if (!Object.prototype.hasOwnProperty.call(this.comments, id)) {
      ...
      if (!isRemoteOperation) {
        this.runtime.emitTargetCommentsChanged(this.originalTargetId, [
          "add",
          id,
          newComment,
        ]);
   ...
   * @param {boolean} isRemoteOperation - set to true if this is a remote operation
   */
  deleteVariable(id: string, isRemoteOperation: boolean) {
    if (Object.prototype.hasOwnProperty.call(this.variables, id)) {
        ...
        for (const target of allTargets) {
          if (
            ...
            const warningText = formatMessage({
              id: "gui.alerts.deletingGlobalVariable",
              default:
                "The variable is used by at least one other sprite, so it can't be deleted",
              description:
                "Message shown when the user attempts to delete a global variable used by other sprites",
            });
            if (!isRemoteOperation) {
   ...
   */
  duplicateVariables(targetId: string, optBlocks?: any): object {
    let allVarRefs;
    if (optBlocks) {
   ...
   */
  setCustomState(stateId: string, newValue: any) {
  ...
  getAllVariableNamesInScopeByType(
    ...
    const stage = this.runtime.getTargetForStage();
    const stageVariables = stage ? stage.getAllVariableNamesInScopeByType(type, false) : [];
   ...
   */
  shareLocalVariableToStage(varId: string, varRefs: Array<object>) {
  ...
  shareLocalVariableToSprite(
    ...
    if (existingLocalVar) {
      ...
      this.runtime.emitTargetVariablesChanged(sprite.originalTargetId, [
        ...
        varType,
        "add",
        { name: newVar.name, value: newVar.value, isCloud: newVar.isCloud },
      ]);
    }
  ...
  resolveVariableSharingConflicts(blocks) {
    // Get all the variable references in the given list of blocks
    const allVarList = this.blocks.getAllVariableAndListWithBlocks(
      blocks,
      true,
    );
    ...
    for (let index = 0; index < allVarList.length; index++) {
      ...
      const maybeOldVarOrList: {
        current: Variable | null;
        stage: Variable | null;
        other: Variable | null;
      } = { current: null, stage: null, other: null };
      const targets = this.runtime.targets.filter((i) => i.isOriginal);

      targets.forEach((target) => {
        const varOrList = target.lookupVariableByNameAndType(
          varName,
          varType,
          true,
        );
      ...
      if (maybeOldVarOrList.stage || maybeOldVarOrList.current) {
        ...
        if (varOrList && varOrList.id !== varOrListField.id) {
  ...
  emitFast(...args: any[]) {
    (this.emit as (...emitArgs: any[]) => boolean)(...args);
  }
}
