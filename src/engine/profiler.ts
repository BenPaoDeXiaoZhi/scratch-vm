 ...
 * @const {Performance}
 */
const performance: any = typeof window === "object" && window.performance;
...
class Profiler {
  ...
  onFrame: (frame: ProfilerFrame) => void;
  START: number;
  STOP: number;
   ...
   */
  constructor(onFrame: (frame: ProfilerFrame) => void = function () {}) {
    /**
   ...
   */
  frame(id: number, arg: any): { count: number } {
  ...
  static available(): boolean {
    return (
      typeof window === "object" && typeof window.performance !== "undefined"
    );
  }
}
 ...
 * @const {number}
 */
(Profiler as any).START = START;

/**
 * A reference to the STOP record id constant.
 * @const {number}
 */
(Profiler as any).STOP = STOP;
