...
class Frames {
      ...
      switch (e.type) {
        case "frame_end_drag":
          ...
          if (e.isOutside) {
            ...
            const newBatchElements = e.batchElements.map((elements) =>
        ...
        case "frame_create":
          if (this.createFrame(e)) {
            this.runtime.emitTargetFramesChanged(targetId, [
              "add",
              e.id,
              this._frames[e.id],
            ]);
  ...
  changeFrame(id, element, value) {
    ...
    switch (element) {
      case "blocks":
        didChange =
          value.blocks.length !== frame.blocks.length ||
