 ...
 */
const isPromise = function (value: any): boolean {
  return (
    value !== null &&
    typeof value === "object" &&
  ...
  if (isHat) {
    ...
    if (
      ...
      if (thread.updateMonitor) {
        const targetId =
          sequencer.runtime.monitorBlocks.getBlock(currentBlockId).targetId;
...
class BlockCached {
  ...
  constructor(blockContainer: any, cached: any) {
    /**
 ...
 */
const _prepareBlockProfiling = function (
  profiler: any,
  blockCached: any,
): void {
 ...
 */
const execute = function (sequencer: any, thread: any): void {
  ...
  if (currentStackFrame.reported !== null) {
    ...
    if (reported.length > 0) {
      ...
      if (lastExisting) {
        i =
          ops.findIndex((opCached) => opCached.id === lastExisting.opCached) +
          1;
      } else {
        i = 0;
      }
