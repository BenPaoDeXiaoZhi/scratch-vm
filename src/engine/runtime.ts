...
import platform from "./tw-platform";
...
import CallbackListGenerator from "../util/callback-list-generator";
import StringUtil from "../util/string-util";
import LogSystem from "../util/log-system";
import Gandi from "../util/gandi";
...
import Scratch3LooksBlocks from "../blocks/scratch3_looks";
import Scratch3MotionBlocks from "../blocks/scratch3_motion";
import Scratch3OperatorsBlocks from "../blocks/scratch3_operators";
import Scratch3SoundBlocks from "../blocks/scratch3_sound";
...
// Polyfill requestAnimationFrame/cancelAnimationFrame so the runtime can
// schedule frames both in browsers and in Node.js environments.
...
type CategoryInfo = {
  ...
  warningTipText?: string;
  blockIconURI?: string;
  menuIconURI?: string;
  color1?: string;
  color2?: string;
  color3?: string;
  onlyVisibleOnShortcut?: boolean;
  blocks: any[];
...
type ExtensionBlockMetadata = any;
...
const defaultBlockPackages = {
  ...
  scratch3_operators: Scratch3OperatorsBlocks,
  scratch3_sound: Scratch3SoundBlocks,
  scratch3_sensing: Scratch3SensingBlocks,
  scratch3_data: Scratch3DataBlocks,
  scratch3_procedures: Scratch3ProcedureBlocks,
};
...
class Runtime extends EventEmitter {
  ...
  /** Interval handle for the optional stepping loop. */
  _steppingInterval: any = null;
  /** @type {!Sequencer} */
  sequencer: Sequencer;
  /**
   ...
   * @type {Array<Thread>}
   */
  _lastStepDoneThreads: Thread[] | null;
  /**
  ...
  currentMSecs: number = 0;
  firingWaitingLoadCallbackQueue: boolean = false;
  asyncLoadingProjectAssets: boolean = false;
  ...
  _registerBlockPackages() {
    for (const packageName in defaultBlockPackages) {
      if (
        Object.prototype.hasOwnProperty.call(defaultBlockPackages, packageName)
      ) {
        // @todo pass a different runtime depending on package privilege?
        const packageObject = new defaultBlockPackages[
          packageName as keyof typeof defaultBlockPackages
        ](this);
        ...
        if ("getHats" in packageObject && packageObject.getHats) {
        ...
        if ("getMonitored" in packageObject && packageObject.getMonitored) {
   ...
   * @private
   */
  _makeExtensionMenuId(menuName: string, extensionId: string): string {
   ...
   */
  removeExtensionPrimitives(extensionId: string) {
    ...
    categoryInfo.blocks?.forEach(({ info }) => {
      if (info.opcode) {
        ...
        if (
          info.blockType === BlockType.EVENT ||
          info.blockType === BlockType.HAT
        ) {
   ...
   * @private
   */
  _convertMenuItems(menuItems: any): any {
  ...
  _convertForScratchBlocks(
    ...
    if (typeof blockInfo === "string") {
      ...
      return this._convertLabelForScratchBlocks({
        text: blockInfo,
  ...
  _convertBlockForScratchBlocks(
    ...
    switch (blockInfo.blockType) {
      ...
      case BlockType.EVENT:
        if (
          !Object.prototype.hasOwnProperty.call(blockInfo, "isEdgeActivated")
        ) {
  ...
  _convertButtonForScratchBlocks(
    ...
    const extensionMessageContext = this.makeMessageContextForTarget(null);
      ...
      let defaultValue =
        typeof argInfo.defaultValue === "undefined"
          ...
          : maybeFormatMessage(
              ...
              this.makeMessageContextForTarget(null),
              undefined,
            ).toString();
    ...
    return this._blockInfo /* powered by xigua start */
      ...
      .map((categoryInfo) => {
        const {
          name,
          color1,
          color2 = Color.darkenHex(color1 || "", 0.1),
        } = categoryInfo;
   ...
   */
  configureScratchLinkSocketFactory(factory: (type: string) => any) {
  ...
  _defaultScratchLinkSocketFactory(type: string): ScratchLinkSocket {
    const Scratch = (self as any).Scratch;
   ...
   */
  registerPeripheralExtension(extensionId: string, extension: object) {
   ...
   */
  attachV2BitmapAdapter(bitmapAdapter: Function) {
  ...
  attachStorage(storage: ScratchStorage) {
    ...
    this.storage.onLoadCostumeError =
      this.storage.onLoadCostumeError || (() => {});
   ...
   */
  _pushThread(id: string, target: Target, opts: any): Thread {
   ...
   */
  _restartThread(thread: Thread): Thread | null {
  ...
  toggleScript(
    topBlockId: string,
    opts: { target?: any; stackClick?: boolean } | null,
  ) {
    opts = Object.assign(
      {
  ...
  addMonitorScript(topBlockId: string, optTarget: Target | null) {
    if (!optTarget) optTarget = this._editingTarget as Target;
    for (let i = 0; i < this.threads.length; i++) {
   ...
   */
  allScriptsDo(f: Function, optTarget?: Target) {
    let targets: Target[] = this.executableTargets;
    if (optTarget) {
      targets = [optTarget];
    }
  ...
  allScriptsByOpcodeDo(opcode: string, f: Function, optTarget?: Target | null) {
    let targets: any[] = this.executableTargets;
    if (optTarget) {
      targets = [optTarget];
    }
    for (let t = targets.length - 1; t >= 0; t--) {
  ...
  startHats(
    ...
    for (const opts in optMatchFields) {
      if (
        !Object.prototype.hasOwnProperty.call(optMatchFields, opts) ||
    ...
    this.allScriptsByOpcodeDo(
      ...
      (script, target) => {
        ...
        if (hatMeta.restartExistingThreads) {
          ...
          for (let j = 0; j < startingThreadListLength; j++) {
            if (
              ...
              sameHatThreadCount++;
              if (
                requestedHatOpcode.startsWith("event_") ||
                requestedHatOpcode.startsWith("control_") ||
                this.runtimeOptions.hatsConcurrency <= sameHatThreadCount
              ) {
        ...
        let opt: { hatParam?: object } | null = null;
        if (hatParam) {
          opt = { hatParam };
        }
  ...
  startHatsWithParams(
    ...
    const fields =
      (data && typeof data.fields === "object" && data.fields) || {};
    const parameters =
      (data && typeof data.parameters === "object" && data.parameters) || {};
   ...
   */
  stopForTarget(target: Target, optThreadException?: Thread | undefined) {
  ...
  stopAll() {
    ...
    for (let i = 0; i < this.targets.length; i++) {
      ...
      if (
        Object.prototype.hasOwnProperty.call(this.targets[i], "isOriginal") &&
   ...
   */
  setIsPlayerOnly(isPlayerOnly: boolean) {
  ...
  findProjectOptionsComment() {
    ...
    const comments = target ? target.comments : {};
  ...
  storeProjectOptions() {
    ...
    if (existingComment) {
      ...
      if (target) {
        ...
        target.createComment(uid(), null, text, 50, 50, 350, 170, false, false);
      }
    }
   ...
   */
  _updateGlows(optExtraThreads?: Array<Thread>) {
    const searchThreads: Thread[] = [];
   ...
   */
  glowBlock(blockId: string | null, isGlowing: boolean) {
    if (isGlowing) {
   ...
   * @param {boolean} isGlowing True to turn on glow; false to turn off.
   */
  glowScript(topBlockId: string | null, isGlowing: boolean) {
    if (isGlowing) {
   ...
   */
  emitFrameDragUpdate(areBlocksOverGui: boolean) {
   ...
   */
  requestAddMonitor(monitor: any, isRemoteOperation: boolean) {
   ...
   */
  requestUpdateMonitor(monitor: any): boolean {
    const id = monitor.get("id");
    ...
    if (deletedMonitor) {
      ...
      if (!isRemoteOperation) {
        this.emitMonitorsChanged([
          "delete",
          monitorId,
          { spriteName: deletedMonitor.get("spriteName") },
        ]);
      }
    }
  }
    ...
    this._monitorState.forEach((monitor) => {
      if (monitor.get("targetId") === targetId) {
        this.emitMonitorsChanged([
          "delete",
          monitor.get("id"),
          { spriteName: monitor.get("spriteName") },
        ]);
      }
    });
  ...
  /**
   * Report that the target has changed in a way that would affect serialization
   * @param {Array} data - An array consisting of roles that have undergone changes.
   */
  emitTargetSimplePropertyChanged(data: any[]) {
   ...
   */
  getLabelForOpcode(extendedOpcode: string): object | undefined {
  ...
  createNewGlobalVariable(
    ...
    this.emitTargetVariablesChanged(stage.originalTargetId, [
      ...
      varType,
      "add",
      { name: newName, value: variable.value, isCloud: variable.isCloud },
    ]);
    return variable;
  }
   ...
   */
  requestTargetsUpdate(target: Target) {
  ...
  /**
   * MengFuzi: 获取独立format message实例
   * @returns
   */
  getFormatMessage(message?: globalFormatMessage.Translations) {
    ...
    return (
      ...
      return formatMessage(msg, args, locales);
    };
  }
  ...
  getGandiAssetFile(fileName) {
    return this.gandi.assets.find(
      (obj) => `${obj.name}.${obj.dataFormat}` === fileName,
    );
  }
  ...
  fireWaitingLoadCallbackQueue() {
    if (this.waitingLoadAssetCallbackQueue.length) {
      ...
      const callbackPromises = this.waitingLoadAssetCallbackQueue.map(
      ...
      Promise.allSettled(callbackPromises)
        .then((queue) => {
          let callbackList: Generator | null = CallbackListGenerator(
            queue.reduce(
              // eslint-disable-next-line no-confusing-arrow
              (acc, res) =>
                res.status === "fulfilled" ? [...acc, res.value] : acc,
              [] as any[],
            ),
          );
          ...
          const stepLoadAssets = async () => {
            ...
            if (result.done) {
              ...
              this.requestAnimationFrameId =
                _requestAnimationFrame(stepLoadAssets);
            } else {
              callbackList = null;
            }
          };
