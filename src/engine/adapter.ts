...
const domToBlock = function (
  ...
  for (let i = 0; i < blockDOM.children.length; i++) {
    ...
    let childBlockNode: any = null;
    let childShadowNode: any = null;
    ...
    switch (xmlChild.name.toLowerCase()) {
      ...
      case "statement": {
        ...
        block.inputs[inputName] = {
          name: inputName,
          block: childBlockNode ? childBlockNode.attribs.id : null,
 ...
 * @return {undefined}
 */
const domToFrame = function (frameDOM: any, frames: any) {
 ...
 */
const domToBlocksOrFrames = function (elementsDOM: any) {
...
const adapter = function (e: any) {
  ...
  return domToBlocksOrFrames(
    parseDocument(e.xml.outerHTML, { decodeEntities: true }).children,
  );
};
