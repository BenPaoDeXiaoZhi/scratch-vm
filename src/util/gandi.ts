...
class Gandi {
  ...
  merge(data: any): any {
    ...
    if (data.wildExtensions) {
      this.wildExtensions = Object.assign(
        this.wildExtensions || {},
        data.wildExtensions,
      );
    }
    if (data.dynamicMenuItems) {
      this.dynamicMenuItems = Object.assign(
    ...
    const isDuplicateId = (id) => this.assets.find((obj) => obj.id === id);
    ...
    newAssets.forEach((obj) => {
      ...
      if (i > 1) {
        log.warn(
          `Duplicate asset found: ${obj.name}.${obj.dataFormat}. Renaming to ${newName}.${obj.dataFormat}`,
        );
        obj.name = newName;
      }
  ...
  serialize(extensions: any): any {
    ...
    if (
      this.dynamicMenuItems &&
      Object.keys(this.dynamicMenuItems).length > 0
    ) {
      gandiObj = Object.assign(gandiObj || {}, {
   ...
   */
  addSpineAsset(key: string, data: any): void {
    if (!this.spine[key]) {
      this.spine[key] = data;
   ...
   */
  addDynamicMenuItem(menuName: string, menuItem: any): void {
   ...
   */
  addWildExtension({ id, url }: any): void {
  ...
  addAsset(asset: any): boolean {
    ...
    if (isDuplicateAsset(asset)) {
      log.warn(
        `addAsset - Duplicate asset found: ${asset.name}.${asset.dataFormat}. Skipping`,
      );
      return false;
    }
