...
const loadGandiAsset = (md5ext, gandiAsset, runtime) => {
    ...
    if (!assetType) {
        log.error(`No asset type found for extension: ${ext}`);
        return Promise.resolve(gandiAsset);
    }

    const isSupported = runtime.gandi.supportedAssetTypes.some(type => type.name === (assetType as any).name);
    if (!isSupported && !runtime.isPlayerOnly) {
        log.error(`unsupported assets type: ${(assetType as any).name} ${md5ext}`);
        return Promise.resolve(gandiAsset);
    }
