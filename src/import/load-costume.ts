...
const loadVector_ = function (
  ...
  return new Promise((resolve) => {
    ...
    if (optVersion && optVersion === 2) {
      throw new Error("svg with optVersion 2 is deprecated by MengFuzi", {
        cause: svgString,
      });
 ...
 */
const readAsImageElement = (src: string): Promise<HTMLImageElement> =>
 ...
 */
const _persistentReadImage = async (
  asset: any,
 ...
 */
const fetchBitmapCanvas_ = function (
  costume: any,
  runtime: any,
  rotationCenter: any[] | null,
): Promise<any> | null {
  ...
  return Promise.all(
    [costume.asset, costume.textLayerAsset].map((asset) => {
      ...
      if (typeof createImageBitmap !== "undefined") {
        if (asset.data.length === 0) {
          asset.data = new Uint8Array([
            ...
            0, 1, 0, 0, 0, 1, 8, 6, 0, 0, 0, 31, 21, 196, 137, 0, 0, 0, 1, 115,
            82, 71, 66, 0, 174, 206, 28, 233, 0, 0, 0, 11, 73, 68, 65, 84, 24,
            87, 99, 96, 0, 2, 0, 0, 5, 0, 1, 170, 213, 200, 81, 0, 0, 0, 0, 73,
      ...
      return new Promise((resolve, reject) => {
        ...
        image.onerror = function () {
          reject(
            `Costume load failed. Asset could not be read, ID: ${asset.assetId}`,
          );
          image.onload = null;
          image.onerror = null;
        };
    ...
    .then(([baseImageElement, textImageElement]) => {
      ...
      if (textImageElement) {
        ...
        canvas.width = (baseImageElement as any).width;
        canvas.height = (baseImageElement as any).height;
        const ctx = canvas.getContext("2d");
        ctx.drawImage(baseImageElement as any, 0, 0);
        ctx.drawImage(textImageElement as any, 0, 0);
 ...
 */
const handleCostumeLoadError = function (
  costume: any,
  runtime: any,
): Promise<any> {
  ...
  runtime.emit("LOAD_ASSET_FAILED", {
    assetType: "costume",
    name: costume.name,
  ...
  const defaultCostumePromise = isVector
    ? loadVector_(costume, runtime, null, null)
    : loadBitmap_(costume, runtime, null);
...
const loadCostumeFromAsset = function (
  ...
  return loadBitmap_(costume, runtime, rotationCenter).catch(
    (error) => {
...
const loadCostume = function (
  ...
  const loadCostumePromise = (asyncLoading?:any) => {
