...
const sanitize = (string: any): string => {
...
class ConstantInput {
   ...
   * @param {boolean} safe
   */
  constructor(constantValue: any, safe: boolean) {
...
const getNamesOfCostumesAndSounds = (runtime: any): Set<string> => {
...
class JSGenerator {
   ...
   */
  variableInputs: Record<string, VariableInput>;
   ...
   * @type {Set<string>}
   */
  namesOfCostumesAndSounds: Set<string>;
   ...
   * @private
   */
  _setupVariables: Record<string, string>;
    ...
    switch (node.kind) {
      ...
      case "args.ccw_hat_parameter":
        return new TypedInput(
          `(thread.hatParam ? thread.hatParam["${sanitize(node.index)}"]: null)`,
          TYPE_UNKNOWN,
        );
    ...
    switch (node.kind) {
      ...
      case "list.delete": {
        ...
        if (index instanceof ConstantInput) {
          // optimize delete at first and last
      ...
      case "looks.changeEffect":
        if (
          Object.prototype.hasOwnProperty.call(this.target.effects, node.effect)
        ) {
      ...
      case "motion.setXY": {
        this.descendedIntoModulo = false;
        const x =
          "x" in node ? this.descendInput(node.x).asNumber() : "target.x";
        const y =
          "y" in node ? this.descendInput(node.y).asNumber() : "target.y";
  ...
  descendVariable(variable) {
    if (
      Object.prototype.hasOwnProperty.call(this.variableInputs, variable.id)
    ) {
   ...
   */
  stopScriptAndReturn(valueJS: string) {
    if (this.isProcedure) {
  ...
  safeConstantInput(value) {
    const unsafe =
      typeof value === "string" && this.namesOfCostumesAndSounds.has(value);
