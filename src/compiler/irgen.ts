...
import * as compatBlocks from './compat-blocks';
 ...
 */
const createVariableData = (scope: 'target' | 'stage', varObj: any): any => ({
    scope,
    id: varObj.id,
    name: varObj.name,
 ...
 * @returns {string}
 */
const generateProcedureVariant = (code: string, warp: boolean): string => {
    if (warp) {
        return `W${code}`;
    }
    return `Z${code}`;
 ...
 */
const parseProcedureCode = (variant: string): string => variant.substring(1);

/**
 * @param {string} variant Variant generated by generateProcedureVariant()
 * @returns {boolean} true if warp enabled
 */
const parseIsWarp = (variant: string): boolean => variant.charAt(0) === 'W';
...
class ScriptTreeGenerator {
     ...
     * @private
     */
    variableCache: Record<string, any>;
    usesTimer: boolean;
    ...
    setProcedureVariant (procedureVariant: string, globalTarget: any): void {
        ...
        let paramNamesIdsAndDefaults: any;
        if (globalTarget) {
    ...
    getBlockById (blockId: string): any {
    ...
    getBlockInfo (fullOpcode: string): any {
     ...
     */
    descendInputOfBlock (parentBlock: any, inputName: string): any {
     ...
     */
    descendStackedBlock (block: any): any {
        switch (block.opcode) {
     ...
     */
    descendSubstack (parentBlock: any, substackName: string): any[] {
     ...
     * @returns {Node[]} List of stacked block nodes.
     */
    walkStack (startingBlockId: string): any[] {
        const result: any[] = [];
        let blockId: string | null = startingBlockId;
     ...
     */
    descendVariable (block: any, fieldName: string, type: '' | 'list'): any {
     ...
     * @returns {*} A parsed variable object.
     */
    _descendVariable (id: string, name: string, type: '' | 'list'): any {
    ...
    descendProcedure (block: any): any {
        ...
        let definitionId: any = blockContainer.getProcedureDefinition(procedureCode);
        let definitionBlock: any;
        let innerDefinition: any;
     ...
     */
    descendCompatLayer (block: any): any {
        this.script.yields = true;
        ...
        const substacks: Record<number, any> = {};
    ...
    readTopBlockComment (commentId: string): void {
    ...
    descendVisualReport (block: any): any {
...
class IRGenerator {
    thread: any;
    blocks: any;
    proceduresToCompile: Map<string, any>;
    compilingProcedures: Map<string, any>;
    /** @type {Object.<string, IntermediateScript>} */
    procedures: Record<string, IntermediateScript>;
    analyzedProcedures: string[];
    ...
    getProcedureDefinitionAndTarget (procedureCode: string): any {
     ...
     */
    generateScriptTree (generator: ScriptTreeGenerator, topBlockId: string): IntermediateScript {
...
export { ScriptTreeGenerator, IRGenerator };
