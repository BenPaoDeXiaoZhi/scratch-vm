...
// @ts-ignore
// Import the package root so Vite resolves it via the `exports.import` field
// (the package does not export its `dist/godirect.min.umd.js` sub-path).
 ...
 * @type {string}
 */
const BLEDataStoppedError: string = 'Force and Acceleration extension stopped receiving data';
...
class GdxFor {
    ...
    _device: null|any;
    _extensionId: any;
    _sensors: { force: number; accelerationX: number; accelerationY: number; accelerationZ: number; spinSpeedX: number; spinSpeedY: number; spinSpeedZ: number; };
    _timeoutID: null|number;
    ...
    _onConnect () {
        ...
        godirect.createDevice(adapter, {open: true, startMeasurements: false}).then((device: any) => {
            ...
            // Enable sensors
            this._device.sensors.forEach((sensor: { setEnabled: (arg0: boolean) => void; }) => {
                sensor.setEnabled(true);
            });

            // Set sensor value-update behavior
            this._device.on('measurements-started', () => {
                const enabledSensors = this._device.sensors.filter((s: { enabled: any; }) => s.enabled);
                enabledSensors.forEach((sensor: { on: (arg0: string, arg1: (s: any) => void) => void; }) => {
...
class Scratch3GdxForBlocks {
    ...
    whenForcePushedOrPulled (args: { PUSH_PULL: any; }) {
    ...
    whenTilted (args: { TILT: any; }) {
        return this._isTilted(args.TILT);
    }

    isTilted (args: { TILT: any; }) {
        return this._isTilted(args.TILT);
    }

    getTilt (args: { TILT: any; }) {
    ...
    _getTiltAngle (direction: string) {
    ...
    getSpinSpeed (args: { DIRECTION: any; }) {
    ...
    getAcceleration (args: { DIRECTION: any; }) {
        switch (args.DIRECTION) {
        case AxisValues.X:
