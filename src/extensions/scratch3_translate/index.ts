...
import fwt from '../../util/fetch-with-timeout';
import languageNames from 'scratch-translate-extension-languages';
import formatMessage from 'format-message';
import getXGAccessCode from '../../util/xg-access-code';
import type Runtime from '../../engine/runtime';
const {fetchWithTimeout} = fwt
/**
 ...
 * @type {string}
 */
const serverURL: string = 'https://translate-service.scratch.mit.edu/';
 ...
 * @type {int}
 */
const serverTimeoutMs: number = 10000; // 10 seconds (chosen arbitrarily).
...
class Scratch3TranslateBlocks {
    ...
    _supportedLanguages: Record<string, string>[];
    _randomLanguageCode: string;
    _translateResult: string;
    _lastLangTranslated: string;
    _lastTextTranslated: string;
    thirdPartApiKey: string;
    host: any;
    isCCWService: boolean=true;
     ...
     * @private
     */
    _getSupportedLanguages (code: string): Array<Record<string, string>> {
        return languageNames.menuMap[code].map((entry: { name: any; code: any; }) => {
    ...
    // powered by xigua start
    _getXiguaSupportedLanguages (code: string) {
    ...
    getViewerLanguage (): string {
        ...
        let langNameObj = names.find(
            (            obj: { code: any; }) => obj.code === this._viewerLanguageCode
        );
        ...
        if (
            ...
            const lookupCode =
                languageNames.scratchToGoogleMap[this._viewerLanguageCode];
            langNameObj = names.find((obj: { code: any; }) => obj.code === lookupCode);
        }
    ...
    getViewerLanguageCode (): string {
        ...
        const languageCode =
            viewerLanguages.reduce((acc, lang) => {
                ...
                if (languageKeys.indexOf((lang as string).toLowerCase()) > -1) {
                    return lang;
                }
                return acc;
            }, '') || 'en';

        return (languageCode as string).toLowerCase();
    }
     ...
     */
    getTranslate (args: any): Promise<any> | any {
    ...
    runWithCCWAPI (args: any, lang: string) {
        const url = this.host;
    ...
    runWithScratchAPI (args: any, lang: string) {
