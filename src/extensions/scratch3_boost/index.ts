...
class BoostMotor {
  ...
  private _pendingDurationTimeoutId: NodeJS.Timeout | null;
  private _pendingDurationTimeoutStartTime: number | null;
  private _pendingDurationTimeoutDelay: number | null;
  private _pendingRotationDestination: number | null;
  private _pendingRotationPromise: (() => void) | null;
   ...
   */
  constructor(parent: Boost, index: number) {
    /**
   ...
   */
  turnOnForDegrees(degrees: number, direction: number) {
   ...
   */
  turnOff(useLimiter: boolean = true) {
    const cmd = this._parent.generateOutputCommand(
      this._index,
      BoostOutputExecution.EXECUTE_IMMEDIATELY,
   ...
   * @private
   */
  _setNewDurationTimeout(callback: () => void, delay: number) {
    this._clearDurationTimeout();
...
class Boost {
   ...
   */
  boostColorForIndex(index: number): string {
   ...
   */
  motor(index: number): BoostMotor | null {
   ...
   */
  setLED(inputRGB: number): Promise<void> {
    const rgb = [
   ...
   * @private
   */
  _registerSensorOrMotor(portID: number, type: number) {
...
class Scratch3BoostBlocks {
  ...
  /**
   * The Boost peripheral instance.
   * @type {Boost}
   * @private
   */
  private _peripheral: Boost;
  ...
  _isTilted(direction) {
    switch (direction) {
      ...
      default:
        return (
          this._getTiltAngle(direction) >= Scratch3BoostBlocks.TILT_THRESHOLD
        );
    }
  }
