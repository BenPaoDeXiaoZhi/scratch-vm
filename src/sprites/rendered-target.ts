...
class RenderedTarget extends Target {
    renderer: any;
    dragging: boolean;
    effects: { color: number; fisheye: number; whirl: number; pixelate: number; mosaic: number; brightness: number; ghost: number; };
    _x: number;
    _y: number;
    direction: number;
    draggable: boolean;
    ...
    initDrawable (layerGroup) {
        ...
        if (!this.isOriginal) {
            this.runtime.startHats(
                'control_start_as_clone', undefined, this
            );
        }
    }
    ...
    renameCostume (costumeIndex, newName, sendNameChangedEvent = true) {
        ...
        if (this.isStage) {
            ...
            for (let i = 0; i < targets.length; i++) {
                ...
                currTarget.blocks.updateAssetName(oldName, newUnusedName, 'backdrop', currTarget.originalTargetId ?? undefined);
            }
        } else {
