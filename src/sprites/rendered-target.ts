...
class RenderedTarget extends Target {
  ...
  initDrawable(layerGroup) {
    ...
    if (!this.isOriginal) {
      this.runtime.startHats("control_start_as_clone", undefined, this);
    }
  }
  ...
  renameCostume(costumeIndex, newName, sendNameChangedEvent = true) {
    ...
    if (this.isStage) {
      ...
      this.blocks.updateAssetName(
        oldName,
        newUnusedName,
        "costume",
        this.originalTargetId,
      );
    }
  }
  ...
  renameSound(soundIndex, newName, sendNameChangedEvent = true) {
    ...
    if (sendNameChangedEvent) {
      this.runtime.emitTargetSoundsChanged(this.originalTargetId, [
        "update",
        this.sprite.sounds[soundIndex].id,
        { name: newUnusedName },
      ]);
    }
  }
  ...
  reorderCostume(costumeIndex, newIndex) {
    ...
    this.runtime.emitTargetCostumeChanged(this.originalTargetId, [
      "reorder",
      [{ id: currentCostume.id, currentIndex: newIndex }],
    ]);
    return true;
  }
  ...
  reorderSound(soundIndex, newIndex) {
    ...
    this.runtime.emitTargetSoundsChanged(this.originalTargetId, [
      "reorder",
      [{ id: sound.id, currentIndex: newIndex }],
    ]);
    return true;
  }
  ...
  duplicate() {
    return this.sprite.duplicate().then((newSprite) => {
      ...
      newTarget.variables = this.duplicateVariables(
        newTarget.id,
        newTarget.blocks,
      );
