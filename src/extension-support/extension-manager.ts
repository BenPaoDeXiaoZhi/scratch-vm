...
import ExtensionWorker from "./extension-worker?worker";
...
class ExtensionManager {
  ...
  builtinExtensions: {
    // This is an example that isn't loaded with the other core blocks,
    // but serves as a reference for loading core blocks as extensions.
    coreExample: () => typeof coreExample;
    // These are the non-core built-in extensions.
    pen: () => typeof pen;
    wedo2: () => typeof wedo2;
    // move to async library
    // music: () => ,
    microbit: () => typeof microbit;
    text2speech: () => typeof text2speech;
    translate: () => typeof translate;
    videoSensing: () => typeof videoSensing;
    ev3: () => typeof ev3;
    makeymakey: () => typeof makeymakey;
    boost: () => typeof boost;
    gdxfor: () => typeof gdxfor;
    // tw: core extension
    tw: () => typeof tw;
  };
  workerMode: string;
  ...
  registerExtension(extensionId, extension, shouldReplace = false) {
    ...
    const extensionInstance = isConstructor(extension)
      ? new extension(this.runtime)
      : extension;
    if (loadedExtServiceName && shouldReplace) {
      const incomingBlocks = extensionInstance.getInfo().blocks; //It only gets block from extension without the menus.
      const incomingMenus = Object.keys(
        extensionInstance.getInfo().menus || {},
      ...
      const opsInUseSet = new Set(
        this.runtime.targets
          .map(({ blocks }) =>
            Object.values(blocks._blocks).map((b) => b.opcode),
          )
          .flat()
      ...
      const oldBlocks = dispatch.callSync(
        loadedExtServiceName,
        "getInfo",
      ).blocks;
      // block type check
   ...
   */
  setGandiExternalExtensionServices(services: Array<string>) {
   ...
   * @private
   */
  async loadGandiExternalExtensionServers(): Promise<any> {
    const allPromises = this._gandiExternalExtensionServices.map((url) =>
      this.loadExternalExtensionToLibrary(url),
    );
  ...
  async loadExtensionURL(
    ...
    if (this.isValidExtensionURL(extFileURL)) {
      ...
      return this.loadExternalExtensionToLibrary(
        extFileURL,
        shouldReplace,
      ).then(({ onlyAdded, addedAndLoaded }) => {
        const allLoader = onlyAdded.map((extId) =>
          this.loadExternalExtensionById(extId, shouldReplace),
        );
        return Promise.all(allLoader).then((res) =>
          res.concat(addedAndLoaded).flat(),
        );
      });
    }
   ...
   */
  async allAsyncExtensionsLoaded(): Promise<any> {
   ...
   */
  refreshBlocks(targetServiceName?: string): Promise<any> {
    const refreshExtension = (serviceName: string) =>
      dispatch
   ...
   */
  onWorkerInit(id: number, e: any | null) {
    ...
    if (
      ...
      const warningTipText =
        ...
        this.runtime.getFormatMessage()({
          ...
          default: "This extension is incompatible with Original Scratch.",
          description:
   ...
   */
  saveWildExtensionsURL(id: string, url: string) {
  ...
  loadExternalExtensionById(extensionId, shouldReplace = false) {
    ...
    return withScratchAPI(this.vm, async () => {
      return this.getExternalExtensionConstructor(extensionId).then(
        (extension) =>
          this.registerExtension(extensionId, extension, shouldReplace),
      );
    });
  }
  ...
  addCustomExtensionInfo(obj, url) {
    ...
    if (this.isExtensionIdReserved(extensionId)) {
      throw new Error(
        `extensionId: '${extensionId}' is reserved in Scratch, please change another one.`,
      );
    }
    if (url) {
      ext.url = url;
  ...
  async getExternalExtensionConstructor(extensionId) {
    ...
    if (typeof func === "function") {
      ...
      if (needRegister) {
        ...
        this.updateExternalExtensionConstructor(
          extensionId,
          needRegister.extensionObject.Extension,
        );
   ...
   */
  async loadExternalExtensionToLibrary(
    url: string,
    shouldReplace: boolean = false,
  ...
  getLoadedExtensionURLs() {
    ...
    Array.from(this._loadedExtensions.keys()).forEach((extId) => {
      const ext =
        this._customExtensionInfo[extId] || this._officialExtensionInfo[extId];
      if (ext && ext.url) {
  ...
  getReplaceableExtensionInfo() {
    ...
    const allReplaceable = Object.values(allExtInfo).filter(
      (ext) => ext.replaceable,
    );
    const allLoaded = Array.from(this._loadedExtensions.keys());
    return allReplaceable.filter((elem) =>
      allLoaded.includes(elem.info.extensionId),
    );
  }
  ...
  replaceExtensionWithId(newId, oldId) {
    ...
    if (diff.size > 0) {
      ...
      throw new Error(
        `opcodes are not fully covered in new extension ${incomingExt.id}`,
        {
      ...
      const typeChangedBlocks = oldExt.blocks.filter((a) =>
        incomingBlocks.find((b) => {
          if (a.info && b.info) {
            return (
              a.info.opcode === b.info.opcode &&
              a.info.blockType !== b.info.blockType
            );
          }
          return false;
        }),
      ...
      if (typeChangedBlocks.length > 0) {
        throw new Error(`extension replace fail new = ${newId} old=${oldId}`, {
          cause: {
            code: "BLOCK_TYPE_CHANGED",
            values: typeChangedBlocks.map((b) => b.info.opcode),
          },
        });
      }
      // do fully replace
      runtime.targets.forEach(({ blocks }) =>
        Object.values(blocks._blocks).forEach((b) => {
