...
export interface ScratchXDescriptor {
    blocks: unknown[][];
...
export interface ScratchXStatus {
    status: 0 | 1 | 2;
    msg: string;
}

const parseScratchXBlockType = (type: string) => {
    if (type === '' || type === ' ' || type === 'w') {
        return {
 ...
 */
const parseScratchXArgument = (argument: string, defaultValue?: any): any => {
    const result: any = {};
...
const wrapScratchXFunction = (originalFunction: (...args: any[]) => any, argumentCount: number, async: boolean) => (args: any[]) => {
 ...
 */
const convert = (name: string, descriptor: any, functions: any) => {
 ...
 */
const createScratchX = (Scratch: any): any => {
    const register = (name: string, descriptor: any, functions: any) => {
     ...
     */
    const getStatus = (extensionName: string): ScratchXStatus => {
