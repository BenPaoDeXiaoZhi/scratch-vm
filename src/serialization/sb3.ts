...
const serializeTarget = function (target, extensions, saveVarId) {
  ...
  obj.costumes = target.costumes
    .filter((item) => !item.isRuntimeAsyncLoad)
    .map(serializeCostume);
  ...
  if (Object.prototype.hasOwnProperty.call(target, "extractProperties"))
    obj.extractProperties = target.extractProperties;
  if (obj.isStage) {
...
const serialize = function (
  runtime,
  targetId,
  { allowOptimization = false, saveVarId = false } = {},
) {
  // Fetch targets
  const obj = Object.create(null);
  ...
  if (targetId) {
    ...
    originalTargetsToSerialize = runtime.targets.filter(
  ...
  const serializedTargets = flattenedOriginalTargets
    .map((t) => serializeTarget(t, extensions, saveVarId))
...
const deserializeBlocks = function (blocks) {
  for (const blockId in blocks) {
    ...
    if (
      ...
      Object.hasOwnProperty.call(block.inputs, "VALUE")
    ) {
  ...
  if (Object.prototype.hasOwnProperty.call(object, "comments")) {
    for (const commentId in object.comments) {
      ...
      const newComment = new Comment(
        ...
        comment.text +
          (typeof comment.extraText === "string" ? comment.extraText : ""),
        comment.x,
        comment.y,
        comment.width,
        comment.height,
...
const deserializeMonitor = function (
  ...
  if (
    ...
    if (!Object.hasOwnProperty.call(target.variables, saveId)) {
      log.warn(
        `Tried to deserialize sprite specific monitor ${monitorData.opcode} but could not find variable ${monitorData.id}.`,
      );
      return;
    }
  }
...
const deserialize = async function (
  ...
  const targetsAssetsPromises = targetObjects.map((target) =>
    parseScratchAssets(target, runtime, zip),
  );
  const gandiAssetsPromises = parseGandiAssets(gandiObjects, runtime);

  const assetsPromisesCount =
    targetsAssetsPromises.reduce(
      (a, t) => a + t.costumePromises.length + t.soundPromises.length,
      0,
    ) + gandiAssetsPromises.length;
  runtime.emit("LOAD_ASSETS_PROGRESS", { total: assetsPromisesCount });

  return (
    Promise.resolve(targetsAssetsPromises)
      ...
      .then((targets) => {
        monitorObjects.map((monitorDesc) =>
          deserializeMonitor(monitorDesc, runtime, targets, extensions),
        );
...
export default {
  ...
  deserializeStandaloneBlocks,
  serializeStandaloneBlocks,
  getExtensionIdForOpcode,
};
