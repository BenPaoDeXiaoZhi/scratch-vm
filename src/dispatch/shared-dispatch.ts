...
class SharedDispatch {
   ...
   */
  call(service: string, method: string, ...args: any[]) {
   ...
   * @returns {Promise} - a promise for the return value of the service method.
   */
  transferCall(service: string, method: string, transfer: any[] | null, ...args: any[]) {
    try {
      ...
      if (provider) {
        ...
        const result = (provider as any)[method].apply(provider, args);
   ...
   * @protected
   */
  _deliverResponse(responseId: number, message: any) {
    try {
  ...
  _onMessage(worker, event) {
    ...
    if (promise) {
      if (typeof message.responseId === "undefined") {
        ...
        promise.then(
          (result) =>
            worker.postMessage({ responseId: message.responseId, result }),
          (error) =>
            worker.postMessage({
   ...
   * @protected
   */
  _getServiceProvider(service: string): {provider: any, isRemote: boolean} | undefined {
    throw new Error(
