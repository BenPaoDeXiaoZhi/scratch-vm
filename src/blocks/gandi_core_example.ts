...
class Scratch3CoreExample {
  ...
  triggerCCWHat(args, util) {
    ...
    util.startHatsWithParams("coreExample_handleCCWHat", {
      parameters: { Msg: args.Msg },
  ...
  updateExtension() {
    ...
    newInfo.blocks = (newInfo.blocks as any[]).concat(dynamicBlock);
    const categoryInfo = this.runtime._blockInfo.find(
      (info) => info.id === this.NS,
    );
    (categoryInfo
      ? this.runtime._refreshExtensionPrimitives
      : this.runtime._registerExtensionPrimitives
    ).bind(this.runtime)(newInfo);
  }
}
