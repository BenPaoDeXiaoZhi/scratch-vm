...
class GandiAsyncAssetManager {
  runtime: Runtime;
  NS: string = "GandiAsyncAssetManager";
  formatMessage: any;
  loadedAssets: any[] = [];
  isAutoClearAsyncCostume = true;
  apis: any;
  ...
  constructor(runtime: Runtime) {
    ...
    this.formatMessage = runtime.getFormatMessage({
      "zh-cn": {
        ...
        "GandiAsyncAssetManager.loadAsyncAsset":
          "加载图片 [IMG] 作为 [TARGET] 的造型, 并 [SHOW]",
      ...
      en: {
        ...
        "GandiAsyncAssetManager.loadAsyncAsset":
          "Load image [IMG] as [TARGET] costume , and [SHOW]",
        "GandiAsyncAssetManager.uploadSnapshot":
          "Upload stage screenshot, for [DESC]",
        "GandiAsyncAssetManager.menu.show": "Show it",
        "GandiAsyncAssetManager.menu.doNothing": "Do Not Show",
        "GandiAsyncAssetManager.autoClearSwitcher.on":
          "Auto Clear When Stop: Enabled",
        "GandiAsyncAssetManager.autoClearSwitcher.off":
          "Auto Clear When Stop: Disabled",
    ...
    return {
      ...
      menus: {
        ...
        showCustomMenu: {
          items: [
            {
              text: this.formatMessage("GandiAsyncAssetManager.menu.show"),
              value: true,
            },
            {
              text: this.formatMessage("GandiAsyncAssetManager.menu.doNothing"),
              value: false,
            },
          ],
        },
  ...
  loadCostume(url) {
    ...
    const isFromCCW =
      imgURL.startsWith("https://m.ccw.site/") ||
      imgURL.startsWith("http://m.ccw.site/");
    if (!isFromCCW) {
    ...
    if (fileExt === null) {
      console.error(
        "load async costume: Invalid file type, must be SVG, PNG, JPG, or JPEG",
      );
      return Promise.reject();
    }
    ...
    return this.runtime.storage
      .load(assetType, this.loadedAssets[assetId], ext)
      .then((asset) => {
        const costumeData = {
          name: "gandiAsyncCostume", // Needs to be set by caller
          dataFormat: ext,
  ...
  loadImageAsCustomBlock(args) {
    this.loadCostume(args.IMG)
      .then((costume) =>
        this.runtime.addAsyncCostumeToTarget(
          costume.md5,
          costume,
          this.runtime.getTargetById(args.TARGET) as object,
          true,
        ),
      )
      .catch((err) => console.error(err));
...
export default GandiAsyncAssetManager;
